...
def capitalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    new_df = df.copy()
    #new_df.columns = new_df.columns.str.upper().where(~new_df.columns.str.contains('e'))
    new_df.columns = new_df.columns.map(str.upper).where(~new_df.columns.str.contains('e'))
    return new_df
...
# %%
'''
Készíts egy függvényt, ahol a bemeneti Dataframe a diákok szülei végzettségi szintjei alapján csoportosításra kerül,
...
def female_top_score(df: pd.DataFrame) -> tuple:
    ...
    new_df['mean'] = new_df.loc[:,['math score', 'reading score', 'writing score']].mean(axis=1)
    new_df = new_df[new_df['mean'] == new_df['mean'].max()]
    maxScore = new_df[['math score', 'reading score', 'writing score']].apply(tuple,axis=1)
    return maxScore
