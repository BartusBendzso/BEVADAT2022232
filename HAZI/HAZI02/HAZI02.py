...
# %%
def column_swap(arr: np.ndarray) -> np.ndarray:
    arr = np.array(arr)
    arr[:,[0,1]] = arr[:,[1,0]]
    return arr
...
# %%
def compare_two_array(arr1: np.ndarray, arr2: np.ndarray) -> np.ndarray:
    return np.array(np.where(np.equal(arr1,arr2)))
...
def get_array_shape(arr: np.ndarray) -> str:
    shapeTuple = arr.shape
    if len(shapeTuple) == 1:
        return "sor: {}, oszlop: {}, melyseg: {}".format(shapeTuple[0],1,1)
    elif len(shapeTuple) == 2:
        return "sor: {}, oszlop: {}, melyseg: {}".format(shapeTuple[0],shapeTuple[1],1)
    elif len(shapeTuple) == 3:
        return "sor: {}, oszlop: {}, melyseg: {}".format(shapeTuple[0],shapeTuple[1],shapeTuple[2])  
...
# %%
def process(n: int, classNumber: int) -> np.ndarray:
    array = np.zeros(classNumber, dtype=int)
    array[n] = 1
    return array

def encode_Y(arr: np.ndarray, classNumber: int) -> np.ndarray:
    arrOut = map(lambda n : process(n,classNumber),arr) 
    return np.array(list(arrOut))
...
>>>>>>> c874593bafd0b3f322d6ff5ea2680787c1ee8fb6
